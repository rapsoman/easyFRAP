...
fid = fopen((strcat('Normalized data - ',handles.folders{i},'.txt')), 'w');
header_info=['time' handles.FRAP_names{i}'];
for j=1:size(handles.FRAP_names{i},1)+1
fprintf(fid,'%s\t',header_info{j});
end
fclose(fid);

xlswrite((sprintf('Normalized data - %s',handles.folders{i})),...
    [handles.t1{i}(:,1) handles.norm1{i}])
...
fid = fopen((strcat('Fitting results - ',handles.folders{j},'.txt')), 'w');
...
for k=1:4
fprintf(fid,'%s\t',header_info2{k});
end
sel_data=handles.FRAP_names{j};
for l=1:size(sel_data,1)
fprintf(fid,'\r\n%s',sel_data{l});
end
fclose(fid);

xlswrite((sprintf('Fitting results - %s',handles.folders{j})),[handles.th{j} handles.mf{j} handles.rsquare{j}]);
