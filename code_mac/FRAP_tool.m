...
[filef,path] = uiputfile('*.csv','Save Fitting Results');
...
cd(path);
filename = strrep(filef, '.csv', '.txt');
fid = fopen(filename, 'w');
header_info1={handles.norm_method handles.fit_eq};
fprintf(fid,'%s\t',header_info1{1});
fprintf(fid,'%s\r\n',header_info1{2});
header_info2={'sample' 't-half' 'mobile_fraction' 'R-square'};
for i=1:4
fprintf(fid,'%s\t',header_info2{i});
end
sel_data=handles.FRAP_names(handles.sfiles);
for j=1:size(sel_data,1)
fprintf(fid,'\r\n%s',sel_data{j});
end
fclose(fid);
% xlswrite(filef,handles.FRAP_names(handles.sfiles),1,'A3')
% xlswrite(filef,{'sample'},1,'A2')
xlswrite(filef ,[handles.th_all handles.mf_all handles.rsquare_all])
...
if handles.flagUpload~=0
[file1,path] = uiputfile('*.csv','Save Raw Data - ROI1');
...
cd(path);
filename = strrep(file1, '.csv', '.txt');
fid = fopen(filename, 'w');
header_info=['time' handles.FRAP_names'];
for i=1:size(handles.FRAP_names,1)+1
fprintf(fid,'%s\t',header_info{i});
end
fclose(fid);
xlswrite(file1 ,[handles.t(:,1) handles.ROI1])
...
if handles.flagUpload~=0
[file2,path] = uiputfile('*.csv','Save Raw Data - ROI2');
...
if handles.flagUpload~=0
[file3,path] = uiputfile('*.csv','Save Raw Data - ROI3');
...
if handles.flagNorm~=0
[file,path] = uiputfile('*.csv','Save Normalized Data');
...
cd(path);
filename = strrep(file, '.csv', '.txt');
fid = fopen(filename, 'w');
header_info=['time' 'mean' 'std' handles.FRAP_names'];
for i=1:size(handles.FRAP_names,1)+3
fprintf(fid,'%s\t',header_info{i});
end
fclose(fid);
    xlswrite(file ,[handles.t1(:,1) handles.mn handles.stdev handles.norm1])
